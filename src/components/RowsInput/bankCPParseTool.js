...
export function parseOPCopyPaste(OPInput) {
  ...
  nonEmptyElements.forEach((field, index) => {
    ...
    if (entry.transceiver) {
      const nextRowStartsNewEntry =
        nonEmptyElements.length > index &&
        nonEmptyElements[index + 1].trim().match(AMOUNT_REGEX);
...
export function parseNordeaCopyPaste(NordeaInput) {
  ...
  const entries = unrefinedRows.map((unrefinedRow) => {
    ...
    const indicator =
      amountWithPlusMinusIndicator.substring(indexOfPlusMinusIndicator) === "+"
        ? ""
        : "-";
    const amount = `${indicator}${amountWithPlusMinusIndicator.substring(
      0,
      indexOfPlusMinusIndicator,
    )}`;
